...
- **`put()` also needed `allowOverwrite: true`**, found by triggering the deployed `/api/cron` a *second* time — the first run succeeds (nothing exists at the fixed path yet), but `@vercel/blob@2.8.0` throws "This blob already exists" on any subsequent write to the same fixed pathname (`addRandomSuffix: false`) without this flag. Both `setStoredRefreshToken` and `setLatestStandings` always overwrite the same path by design (daily refresh, rotated token), so this is required, not optional. This is the fourth distinct real-API behavior (after the cache-control minimum, the `BlobNotFoundError` shape, and the `addRandomSuffix` default) that the fully-mocked test suite structurally cannot catch, since `put`/`head` are mocked and never exercise Vercel's actual validation rules — confirmed working only by manually re-invoking the live deployment four times in a row.
- **Local OIDC tokens are scoped to "development," not Production** — `vercel env pull` always mints a dev-scoped `VERCEL_OIDC_TOKEN`, and Vercel Blob's OIDC integration isn't enabled for that environment by default, so `setup-yahoo-auth.ts` cannot seed Blob storage when run locally (`BlobOidcEnvironmentNotAllowedError`). This turned out not to matter: `api/cron.ts` already falls back to `YAHOO_INITIAL_REFRESH_TOKEN` when Blob is empty, and its first real invocation (genuine Production OIDC) writes the encrypted, rotated token to Blob itself. The setup script still gets you the initial refresh token and the league/team key lookup (once Yahoo approves access) — it just can't do the Blob write itself locally, and doesn't need to.
